...
void udelay(unsigned long usec)
{
	...
	sdramc_ck_delay(((usec * (get_hsb_clk_rate() + 999999)) / 1000000UL));
}

static int set_interrupt_handler(unsigned int nr, void (*handler)(void), unsigned int priority)
{
	...
	if ((handler_addr & HANDLER_MASK) != handler_addr || (priority & INTLEV_MASK) != priority)
		return -EINVAL;
